...
def plot_tips_treasury_spreads(
    ...
    """
    Plot TIPS-Treasury spreads over time using Plotly.

    Parameters:
        data_df (pd.DataFrame): DataFrame containing arbitrage data
        start_date (str): Start date in format 'YYYY-MM-DD' (optional)
        end_date (str): End date in format 'YYYY-MM-DD' (optional)
        save_path (str): If provided, save figure to this path as HTML
    ...
    # Apply date filter
    if start_date and end_date:
        plot_df = data_df.loc[start_date:end_date]
    elif start_date:
        plot_df = data_df.loc[start_date:]
    elif end_date:
        plot_df = data_df.loc[:end_date]
    else:
        plot_df = data_df
    ...
    for col in plot_df.columns:
        fig.add_trace(
            go.Scatter(
                ...
                y=plot_df[col],
                mode="lines",
                name=legend_name_map.get(col, col),
            )
        )

    title_dates = f"({start_date[:4] if start_date else ''}-{end_date[:4] if end_date else ''})"
    fig.update_layout(
        title=f"TIPS Treasury Arbitrage Spreads {title_dates}",
        xaxis_title="Date",
...
if __name__ == "__main__":
    ...
    fig_path = f"{OUTPUT_DIR}/tips_treasury_spreads.html"
